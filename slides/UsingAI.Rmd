...
```{r img-context1, echo=FALSE, fig.cap = "ChatGPT giving its best guess", out.width = "80%"}
...
`summary()` provides descriptive statistics this is useful because it provides a list of all variables with some descriptive statistics so that the AI has a sense of the type and range of data:
...
```{r, echo=FALSE, fig.cap="Adding comments"}
knitr::include_graphics("images/gpt-8.png")
```
...
```{r, echo=FALSE, fig.cap="Adding comments"}
knitr::include_graphics("images/gpt-10.png")
```
...
penguins <- penguins %>%
  mutate(size = if_else(body_mass_g > median_body_mass, "large", "small"),
         chonk_category = case_when(
           size == "large" & sex == "male" ~ "chonky_boi",
           size == "large" & sex == "female" ~ "chonky_gurl",
           size == "small" & sex == "male" ~ "smol_boi",
           size == "small" & sex == "female" ~ "smol_gurl",
           TRUE ~ NA_character_
         ))
...
- Works out of the bag

- Used old training data: TRUE superseded by .default

- No need to set NA "type"
...
```{r, echo=FALSE, fig.cap="Adding comments"}
knitr::include_graphics("images/gpt-11.png")
```
...
```{r, echo=FALSE, fig.cap="Adding comments"}
knitr::include_graphics("images/gpt-12.png")
```

- Pretty good

- Suggests formal tests of model fit rather than diagnostic plots
