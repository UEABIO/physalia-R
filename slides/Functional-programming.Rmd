...
Purpose: Speed up computation by splitting tasks across multiple processor cores.

Key Concept:

- A regular for loop runs tasks sequentially (one after another).

- A parallel for loop runs tasks concurrently across multiple cores, reducing total computation time.
Main Package:

`foreach` package with doParallel backend is commonly used to achieve parallel loops.
...
detectCores()  # Detect available cores

cl <- makeCluster(4) 
registerDoParallel(cl)            # Register parallel backend with 4 cores


results <- foreach(i = 1:10, .combine = c) %dopar% {
   i^2  # Task to run in parallel
}
stopCluster(cl)  # Stop cluster after the loop
